------------------------------------------------------------------------------------
 -- PolicyIDMetadata Snapshot
 --  this query must be run prior to running the vote_results_policyid_metadata.sql script
 --  it takes a proposal_id as input and will take a snapshot for the associated
 --  policyid an epoch, it also gets the metadata of each token
 ...
 drop table if exists tmp_voteaire_snapshot;

 WITH constants as (
     select snapshot_epoch::int as epoch_no,
 	policy_id as policyid
     from voteaire_proposals
     where proposal_id = 'c2d5cc39-0a7a-4d5a-9b90-8856cc329e02'
 )
 select 
     b.block_no,
     b.epoch_no,
     ...
     (tx_metadata.json->c.policyid->convert_from(ma.name, 'utf8')) as metadata,
     o.address, 
     o.stake_address_id,
 ...
 from tx_out o
     ...
     inner JOIN ma_tx_mint mm ON mm.ident = mo.ident
     inner JOIN tx_metadata ON tx_metadata.tx_id = mm.tx_id
